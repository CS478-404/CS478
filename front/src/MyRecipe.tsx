...
function MyRecipe() {
    let [created, setCreated] = useState<Meal[]>([]);
    let [error, setError] = useState<string | null>(null);
    let renderLimit = 24;
    const [page, setPage] = useState<number>(1);
    let visibleCreated = created.slice(0, renderLimit);

    let navigate = useNavigate();

    useEffect (() => { 
        async function fetchCreated() {
            let createdMeals = await axios.get('/api/created');
    ...
    return (
        <div style={{ width: "100%" }}>
            ...
            <Box sx={{ width: "100%" }}>
                ...
                <Pagination
                    page={page}
                    count={Math.ceil(visibleCreated.length / renderLimit)}
